...
async function saveReviewLesson(status){var a=_reviewActive();if(!a.lesson)return false;syncReviewLessonDocumentContent(true);var publishing=status==='published';a.lesson.title=(document.getElementById('review-lesson-title').value||a.lesson.title||'Bài giảng ôn tập').trim();a.lesson.status=publishing?'published':'draft';var doc=_reviewDoc(a.lesson,true);a.lesson.blocks=[doc];a.lesson.content=doc.content||'';a.lesson.type='editor';if(publishing)a.details.status='published';var idToken=publishing?await AuthModule.getIdToken():'';var result=await DB.saveSubjectDetails(_reviewLessonSubjectId,a.details,!publishing,idToken);renderReviewLessonTree();if(publishing&&!result.ok){showToast('Chưa đăng công khai: '+(result.sync&&result.sync.reason||'không thể đồng bộ máy chủ')+'.','error');}else if(publishing){showToast('Đã đăng công khai bài giảng.','success');}else{showToast('Đã lưu nháp trên thiết bị này. Dùng “Đăng công khai” để sinh viên xem.','success');}return result;}
