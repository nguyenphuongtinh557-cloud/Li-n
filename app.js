...
function safeStudyReaderRichText(html) {
  ...
  const allowed = new Set(['P','BR','STRONG','B','EM','I','U','UL','OL','LI','H2','H3','H4','BLOCKQUOTE','PRE','CODE','A','IMG','TABLE','THEAD','TBODY','TR','TH','TD','VIDEO','SOURCE','IFRAME']);
...
function syncReviewLessonDocumentContent(flush){clearTimeout(_reviewLessonDocumentTimer);var sync=function(){var l=_reviewActive().lesson;if(l)_reviewDoc(l,true).content=_reviewLessonDocumentEditor?_reviewLessonDocumentEditor.getContent():(document.getElementById('admin-review-lesson-document-editor')||{}).value||''};if(flush)sync();else _reviewLessonDocumentTimer=setTimeout(sync,300);}
...
function saveReviewLesson(status){var a=_reviewActive();if(!a.lesson)return Promise.resolve(false);syncReviewLessonDocumentContent(true);a.lesson.title=(document.getElementById('review-lesson-title').value||a.lesson.title||'Bài giảng ôn tập').trim();a.lesson.status=status==='published'?'published':'draft';var doc=_reviewDoc(a.lesson,true);a.lesson.blocks=[doc];a.lesson.content=doc.content||'';a.lesson.type='editor';return DB.saveSubjectDetails(_reviewLessonSubjectId,a.details).then(function(result){renderReviewLessonTree();showToast(result.ok?'Đã lưu bài giảng.':'Đã lưu cục bộ.',result.ok?'success':'info');return result});}
