...
async function summarizeChunkDeep({ chunkText, docTitle, chunkId, totalChunks, onProgress }) {
  ...
  onProgress?.(`Đang trích xuất kiến thức chuyên sâu ${chunkLabel}...`);

  let result;
  try {
    result = await callGeminiJSON(DEEP_CHUNK_PROMPT(chunkText, docTitle, chunkLabel), DEEP_CALL_OPTS);
  } catch (err) {
    throw new Error(`chunk-${chunkId}-failed: ${err.message}`);
  }

  const check = result?.self_check || {};
  const fidelity = Number(check.quality?.source_fidelity) || 0;
  const coverageAvg = _avg(Object.values(check.coverage || {}));

  if ((fidelity < 75 || coverageAvg < 75) && Array.isArray(result?.topics)) {
    onProgress?.(`Đang trích xuất lại ${chunkLabel} (độ trung thực/độ phủ chưa đạt)...`);
    try {
      const retried = await callGeminiJSON(DEEP_RETRY_PROMPT(chunkText, docTitle, chunkLabel, result, check), DEEP_CALL_OPTS);
      if (Array.isArray(retried?.topics)) result = retried;
    } catch {
  ...
  return {
    ...
    coverage: _normalizePct(result?.self_check?.coverage),
    quality: _normalizePct(result?.self_check?.quality)
  };
}
